...
- **Enemy art is primitive but fully dressed.** The ENTIRE roster now has an armored, glowing-visor look (decorative child meshes; scripts untouched): base enemy + boss (pauldrons + visor + reactor core), brute/medic/ranged (themed visor red/green/purple + shoulders + core), turret/sniper (glowing lens/scope eye), bomber (bright armed-core eye). Reads as designed threats, not bare capsules. Final goal is still rigged character models (same pattern as the player GLB) — the current primitives are the marked-for-replacement placeholder.
