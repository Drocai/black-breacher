...
**Two-mission campaign (June 2026):** the loop is now a campaign. `Game.ARENA_SCENES = ["res://main.tscn", "res://main2.tscn"]` + `scene_for_current_mission()` / `is_final_mission()` drive the flow; `objective.gd::_advance` loads the next arena (or `victory.tscn` once done) instead of reloading. **`main2.tscn`** is a distinct "Cell Block" arena — wider (walls ±7), cold blue lighting, a central vault-over `Barrier`, a breachable back `Door2` gate, 4 waves, nav bakes 62 polys. **`victory.tscn/.gd`** is the campaign-complete screen (final stats; ENTER → title). **Between-wave upgrades:** `wave_manager` runs a resupply intermission (`intermission_time`) that drops a rotating **`upgrade_pickup`** (VITALITY/PLATING/ORDNANCE/ADRENALINE) at the arena centroid; `player.apply_upgrade(kind)` applies +maxHP / +armor / +grenades / +speed. To add a mission: append a scene to `Game.ARENA_SCENES` AND bump `ARENA_COUNT` in the playthrough driver.
