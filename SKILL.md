...
- **Enemies are becoming REAL rigged character models** (CC0 KayKit Adventurers in `characters/`: Knight/Barbarian/Mage/Rogue/Rogue_Hooded — self-contained GLBs, embedded textures, 76 shared baked animations). DONE: base enemy = Knight (`enemy.tscn`/`enemy.gd`), brute = Barbarian (`brute.tscn`/`brute.gd`) — both reworked to instance the GLB and drive its AnimationPlayer from state (Idle/Run/Attack/Hit/Death) while keeping all AI/physics. Pattern to convert the rest: swap the capsule Mesh for a `Model` GLB instance, find its AnimationPlayer, replace material-flash/position-tweens with `_play_anim` calls (ANIM consts), `_anim_lock` so attack/hit/death own the model. STILL primitive (with the old dressed-capsule look): medic, enemy_ranged, turret, sniper, bomber, boss — convert medic/ranged/fast to Mage/Rogue, boss to a scaled-up character. KayKit anims that exist: Idle, Walking_A/B, Running_A/B, 1H/2H/Dualwield/Unarmed_Melee_Attack_*, 1H_Ranged_Shoot, Hit_A/B, Death_A/B, Block, Dodge_*, Jump_*.
