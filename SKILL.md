...
- **Enemy art is primitive but dressed.** The base enemy (melee/heavy/fast, `enemy.tscn`) and the boss (`boss.tscn`) now have an armored silhouette — shoulder pads/pauldrons + a glowing red visor (eyes) + chest/reactor core (emissive, pops via bloom) — so they read as menacing armored figures, not bare capsules. STILL bare primitives (candidates for the same decorative-children treatment): bomber, brute, medic, turret, sniper, enemy_ranged. Final goal is rigged models (same pattern as the player GLB).
