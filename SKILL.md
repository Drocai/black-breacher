...
**Navmesh SOLVED (June 2026):** CSG geometry won't bake via the scene parser (0 polys). Fix that works: `nav_baker.gd` on a `NavigationRegion3D` builds **procedural source geometry** — walks the level, adds a `BoxMesh` for every collidable `CSGBox3D`/`CSGCylinder3D` at its global transform into a `NavigationMeshSourceGeometryData3D`, then `NavigationServer3D.bake_from_source_geometry_data` → **44 polys** (validated). Base melee `enemy.gd` (melee/heavy/fast) now paths via a `NavAgent` (`NavigationAgent3D`) → `get_next_path_position`, with direct-bearing + sidestep fallback if no path. (Same pattern can extend to bomber/brute/medic.) The "parse RenderingServer meshes" warning at bake is a harmless one-time perf note.

**Multi-phase BOSS (June 2026):** the climax — **THE WARDEN** (`boss.tscn`, 60 HP, groups "enemy"+"boss", exposes `health`/`max_health`/`boss_name`). Timer-driven state machine (no awaits on freed nodes): **Phase 1** chase + telegraphed AoE slam; **Phase 2** (33–66%) + projectile volley + summons 2 `enemy.tscn` adds; **Phase 3** (<33%) enrage + charge. Stagger-resistant; 2000pt kill + multi-explosion death. **Boss health bar** (`boss_healthbar.tscn`, top-center, reads group "boss") shown only when a boss is alive. Replaces the old scaled-grunt boss; instanced at scale 1.2 to fit the room. Built by 2 parallel agents, integrated centrally.

**Multi-agent batch 3 (June 2026):** 4 parallel agents → **Medic** (`medic.tscn`, heals nearby enemies on a pulse — kill it first; wave 2+), **Proximity Mine** (`mine.tscn`, arms when player nears + detonates; placed + breakable), **Sniper** (`sniper.tscn`, telegraphs a red beam ~1.1s then fires heavy — dodge out of the line; wave 3+), **Grenade** (`grenade.tscn`, `launch(dir,force)`) + **Grenade pickup** (`grenade_pickup.tscn` → `player.add_grenades`). Integration: player grenade system (**G** to throw, `var grenades`, `add_grenades`, `GRENADE_SCENE` preload); **score COMBO** in `Game` (consecutive kills → `combo_mult()` x1–x5, 3s window, `_process` decay, reset on arena reset); HUD shows `GRENADES`/`ARMOR`/`COMBO`; mines + grenade pickups placed. Enemy roster: melee/fast/heavy/ranged/bomber/turret/brute/medic/sniper + boss (10 types).

**Multi-agent batch 2 (June 2026):** 3 more parallel agents → **Explosion FX** (`explosion.tscn`, reused via new `Game.spawn_explosion(pos)`, called from bomber + barrel detonations), **Brute** (`brute.tscn`, armored: `take_hit` resists damage < `armor_threshold` (light jabs) and takes full from heavy moves — break brutes with kick/halligan/special/finisher; wired into waves), **Armor pickup** (`armor_pickup.tscn` → `player.add_armor`; player has `var armor` that soaks damage before health in `take_damage`, reset on respawn; HUD shows `ARMOR n`). 2 armor pickups placed (approach + arena). Enemy roster now: melee/fast/heavy/ranged/bomber/turret/brute + boss.

**Multi-agent combat assets (June 2026):** built 3 new assets via 3 parallel sub-agents (each writing only its own new files; main thread integrated + validated centrally — conflict-safe pattern). **Bomber** (`bomber.tscn`, rushes + 0.6s fuse + AoE explode), **Turret** (`turret.tscn`, stationary, fires `projectile.tscn`), **Barrel** (`barrel.tscn`, group "breakable", explodes damaging enemies). Bomber/Turret wired into `wave_manager` spawn kinds (bomber wave 2+, turret wave 3+); 2 barrels placed in the arena. Gotcha learned: validating a script via `godot --script` reports false "Identifier not found: Game" because autoloads aren't loaded in that mode — validate via a real scene run (`wave_manager` `preload`s force-compile the new scripts with autoloads present).
...
**Navmesh SOLVED (June 2026):** `nav_baker.gd` (on the `NavRegion` node in main.tscn) builds `NavigationMeshSourceGeometryData3D` *procedurally* by walking the scene and emitting a BoxMesh per CSG collider (CSGBox3D→its size, CSGCylinder3D→a bounding box), then `NavigationServer3D.bake_from_source_geometry_data` → **44 polys** (was 0 over raw CSG). Every chasing enemy now paths around walls/crates: `enemy.gd` (melee/heavy/fast), `bomber.gd`, `brute.gd` each have a `NavAgent` (NavigationAgent3D) child and steer via `get_next_path_position()`, falling back to a direct bearing + sidestep when no path exists. **Medic stays on direct steering** (it flees the player, so to-player pathing would be inverted). One benign one-time bake warning ("parse RenderingServer meshes at runtime").

**Feel/FX + impact audio pass (June 2026):** reusable **`shockwave.tscn/.gd`** — a flat expanding ground ring (TorusMesh, additive unshaded glow, per-instance material, self-frees) spawned via **`Game.spawn_shockwave(pos, color, scale)`**. New **`Game.spawn_sound_3d(pos, path, vol, pitch)`** fire-and-forget positional one-shot. Three synthesized SFX (16-bit mono 44.1k, LFS): `flashbang.wav` (concussive bang), `breach_impact.wav` (deep splinter thud), `heavy_step.wav` (sub-bass footstep for his size). Wired: **explosions** (→ ring + bang, so boss slams that route through `spawn_explosion` get it free), **door breach** (thud + threshold ring), **landed halligan** swings, **execution finisher** (ring + thud), and **every footstep** layers the sub-thump. All headless-validated (FX/audio paths instantiate + play, main.tscn loads clean).

**Three-mission campaign (June 2026):** the loop is a campaign. `Game.ARENA_SCENES = ["res://main.tscn", "res://main2.tscn", "res://main3.tscn"]` + `scene_for_current_mission()` / `is_final_mission()` drive the flow; `objective.gd::_advance` loads the next arena (or `victory.tscn` once done) instead of reloading. **`main2.tscn`** = "Cell Block" — wider (walls ±7), cold blue lighting, central vault-over `Barrier`, breachable back `Door2`, 4 waves, 62 nav polys. **`main3.tscn`** = "Cargo Yard" — larger/longer (walls ±8, back at z=-32), warm orange hazard lighting, tall shipping-`Container` cover, back gate, 5 waves, 91 nav polys. Upgrade pickups flash a HUD toast on pickup (`Game.show_toast`/`toast_active`, set by `player.apply_upgrade`). **Per-arena enemy bias:** `wave_manager` has an `enemy_bias` export (`_apply_bias` re-flavors ~half each wave) — main2="bruiser" (brutes/heavies), main3="ranged" (ranged/snipers/turrets), main1="" balanced — so each mission fights differently, not just looks different.

**World color pass (June 2026 — "feel like a world, not a prototype"):** each arena gets emissive accent dressing that pops via the glow/bloom — glowing wall LED strips, floor edge-lines, wall screen-panels, a painted hazard floor stripe, and a glowing center-line "runway". All non-colliding decorative (navmesh unchanged 44/62/91). **Distinct palette per arena** for world variety: Hangar = amber + cyan, Cell Block = cyan + cold white, Cargo Yard = red warning + green. Materials defined inline per scene (`Mat_led_*`, `Mat_hazard`). Reads as a lived-in designed facility instead of a grey box.

**Cinematics + look (June 2026 — "make him read HUGE"):** `camera_rig.gd` is a director: `shake`, `punch_in`, **`fov_kick(amount,dur)`** (visceral FOV punch), **`hero_angle(dur)`** (dips low + tilts up to read his height) — effects decay in scaled time so they linger through slow-mo. **Time dilation is centralized in `Game`** (`_dilate` is ref-counted by id so overlapping hitstop/slow-mo never restore early; only the latest owns the return to 1.0): `Game.hitstop()` / `Game.slowmo()`; `player._hitstop` delegates. **Signature SEISMIC SLAM (Z)** = `player._try_seismic_slam`/`_apply_seismic_slam`: AoE damage + stagger + outward knockback, big shockwave + explosion FX + breach thud, wrapped in slow-mo + heavy shake + FOV kick + low-angle. Door breach / halligan / execution finisher upgraded to the richer camera. **Aerial ground-pound:** pressing SLAM (Z) while airborne (after a jump) starts a committed dive straight down (`_air_slamming`, `air_slam_speed`); on landing `_apply_seismic_slam(true)` fires a BOOSTED slam — 1.5× radius, 2× damage, bigger shake/FOV/slow-mo. Rewards jump→slam, amplifies his crushing weight. **First-person mode (B):** `camera_rig.set_first_person` rides his eyeline (`fp_head_height`) and looks DOWN (`fp_look_down`) so small enemies sit below you (you feel huge), with a heavy weight-bob (`fp_bob_amount`) while moving; shake/FOV-kick still land (dampened). His **body stays visible** in FP — head sits above the downward frame, torso/shoulders/arms (+halligan) fill the lower view, so melee is readable and his bulk reads. Third-person stays default. **Directional damage indicator:** `take_damage` records `Game.hit_dir` (world dir to nearest enemy); the HUD points a hot orange-red wedge from center toward the threat (camera-relative), and the full-screen hit flash was tuned down (0.45→0.32) so the wedge reads over it. **Boss eruption (THE WARDEN, phase 2+):** `_do_eruption` marks the player's spot with a warning ring, then `_erupt_at` explodes there after `eruption_delay` — a dodge-the-tell AoE distinct from the proximity slam. **Signature BREACHER CHARGE (T)** = `_try_charge`/`_charge_sweep`: an unstoppable shoulder-charge (dash in aim/facing dir) that plows the crowd (each enemy hit once, blasted aside), shatters breakables in-lane, grants brief i-frames. **Enemy wall-splat** (`enemy.gd::_wall_splat`): a staggered enemy hurled into a wall above a speed threshold crunches (spark + bonus damage + camera kick), so the Charge/Slam knockbacks land. **Per-arena music** (`music_hangar`/`music_cellblock`/`music_cargo`, seamless 16s synth loops, LFS) on each arena's Music node. **Graphics post** per arena WorldEnvironment: ACES tonemap, glow/bloom (emissives pop), SSAO, depth fog tinted per mood, contrast+saturation grade. The playthrough bot fires one Seismic Slam per run to exercise the slow-mo/camera path; all feel/look values are exported for one-playtest tuning. **`victory.tscn/.gd`** is the campaign-complete screen (final stats; ENTER → title). **Between-wave upgrades:** `wave_manager` runs a resupply intermission (`intermission_time`) that drops a rotating **`upgrade_pickup`** (VITALITY/PLATING/ORDNANCE/ADRENALINE) at the arena centroid; `player.apply_upgrade(kind)` applies +maxHP / +armor / +grenades / +speed. To add a mission: append a scene to `Game.ARENA_SCENES` AND bump `ARENA_COUNT` in the playthrough driver.

**Visual verification (June 2026):** `tools/capture/` runs Godot **WINDOWED** (NOT `--headless` — headless can't render) via `capture_bootstrap.tscn`, drives `main.tscn` into a posed beat, and saves rendered viewport PNGs (`user://bb_start.png` / `bb_combat.png`, under `%APPDATA%\Godot\app_userdata\Breacher\`) via `get_viewport().get_texture().get_image().save_png()` after `await RenderingServer.frame_post_draw`. This is how to actually SEE the game — the playthrough test only proves logic. First use confirmed the game renders correctly AND caught that the protagonist read at normal human size; the **breacher mesh node is now scaled 1.55× uniform** in all three arenas (visual only, collision/logic untouched) so he towers over enemies (verified by screenshot — their heads reach his chest, feet stay grounded). Subsequent screenshot-verified passes: **mood** (dimmed ambient ~0.28, exposure ~0.9, tighter/lower room lights → dark "the Black" atmosphere, emissives pop, still readable, per-arena color kept); **camera framing** (`camera_rig` offset 3.4/5.8→2.9/5.2, pitch −28→−22 so he reads more imposing); **environment detail** (dark-metal ceiling fixtures + wall pipes per arena — decorative `use_collision` off, navmesh unchanged 44/62/91). Use `tools/capture` to re-verify any look change; kill stray Godot procs before the headless playthrough (a windowed capture left running starves it → false timeout).

**Performance baseline (June 2026, MEASURED):** `tools/perf/` runs main.tscn WINDOWED, drives a worst-case effects storm (explosion+dust+debris+shockwave+sound every 0.2s + enemies), samples real rendered FPS (vsync off, uncapped). Result: **avg ~563 / min ~485 FPS** — ~8x headroom over 60. Desktop runtime is NOT a bottleneck; effects are cheap. Re-run after any heavy FX/particle change. Caveat: measures DESKTOP native; the web build on a mobile GPU is the real constraint (see below).

**Remaining temporary systems / placeholders (Definition-of-Done tracking):**
- **Enemy ART DIRECTION (IMPORTANT):** the game is a GRITTY REALISTIC MODERN TACTICAL breacher game (ex-Green-Beret / black-ops). Enemies must be believable adult tactical combatants (operators / SWAT / mercs / PMC / cartel). NO fantasy/medieval/knight, NO stylized low-poly/Roblox/chibi, NO childlike proportions. A KayKit (fantasy knights) experiment was tried and REVERTED for tone — it lives on branch `kaykit-enemies-experiment` (do not merge). Current enemies are the neutral dressed-capsule placeholder (abstract, tonally inoffensive). The model+AnimationPlayer controller pattern from that experiment is sound and reusable for a REALISTIC rig: instance a `Model` GLB, find its AnimationPlayer, drive `_play_anim` by state with `_anim_lock`. **Do NOT import character assets on name alone — verify style/rig/scale/animations/thematic fit (and licensing: prefer CC0/permissive) and get user approval first.** Realistic free options are scarce (most are paid Unity/Unreal packs); Mixamo (realistic rigged humans + tactical anims, free) needs interactive Adobe auth + FBX→GLB.
- **`atmosphere.tscn/.gd`** — a subtle looping dust-mote field (additive, dim, sparse, `tint` export) instanced in each arena (warm/cold/warm) so the air feels alive. Cosmetic, no gameplay effect.
- **No jump animation clip** — Space jumps but the body doesn't animate the leap; needs a Meshy/Blender jump export wired into `_update_locomotion_anim`.
- **Web build first-load is ~88MB** (38MB wasm engine fixed + 20MB breacher.glb + audio ~4.5MB + textures). DONE: character albedo/normal/roughness downscaled to 1K via `process/size_limit=1024` in their .import → pck 58→50MB (-8MB), no visible quality loss at gameplay distance. Remaining candidates (NOT done — higher risk / unverifiable headless): compress audio import to QOA (needs a music.gd loop-calc fix since it assumes 16-bit PCM `data.size()/2`; audio result can't be heard-verified headless), and strip unused animations from breacher.glb in Blender (biggest single lever at 20MB but risks the rig/clips the game uses). Desktop FPS is fine regardless.
- **Feel values are exported but NOT playtest-tuned** — slow-mo durations (`Game.slowmo`), shake amplitudes, FOV-kick amounts, charge speed/range, seismic radius, hit-flash intensity. All `@export`/tunable; need ONE human F5/web playtest to dial. Marked for replacement-by-tuning.
- **Directive said "first-person"; game is third-person** by deliberate decision (the "his bigness reads everywhere" identity requires seeing him). Flagged to the user; awaiting confirmation before any FP mode.

**Web build / playable in-browser (June 2026):** `tools/build_web.ps1` exports a single-threaded HTML5 build to `build/web` (`-Serve` to host at :8765). Requires the Godot 4.7 export templates installed at `%APPDATA%\Godot\export_templates\4.7.stable\` (download `Godot_v4.7-stable_export_templates.tpz` ~1.28GB from the godot-builds 4.7-stable release and extract `templates/*` there, or use the editor's Manage Export Templates). Preset is threads-OFF (no SharedArrayBuffer → any static host, no COOP/COEP). `export_presets.cfg` is gitignored; the script regenerates it. **VERIFIED** the build boots in-browser (console: `Godot Engine v4.7`, `WebGL 2.0`, `[BB] boot`). `build/` is gitignored (large artifact). Public deploy is NOT automated (outward-facing — needs explicit user go-ahead; Vercel CLI is installed + authed as `drocai`).

**Automated playthrough test (June 2026):** `tests/playthrough/` is a headless regression test that drives the REAL campaign — EVERY arena in `Game.ARENA_SCENES`, in sequence, grabbing resupply drops — through to the `victory.tscn` screen, asserting `missions_cleared` advanced by `ARENA_COUNT`, kills increased, `score>0`. A `TestDriver` parented under the SceneTree root survives `change_scene_to_file`, so one arena-agnostic per-frame loop handles every arena. **Grade by the driver's process exit code (`quit 0`=pass / `1`=fail), NOT by grepping stdout** — the final print can be lost to console-wrapper pipe buffering on a fast exit. `tools/run_playthrough.ps1` does this. Last run: PASS, arenas=2 kills=33 score=29600, both bosses down, all 4 upgrade kinds applied, Victory reached. A `TestDriver` node is parented under the SceneTree root so it survives `change_scene_to_file`. Run with **`tools/run_playthrough.ps1`** (finds the bundled Godot, exit 0=PASS / 1=FAIL). Proves the loop COMPLETES, not just compiles — last run: `PLAYTHROUGH_OK kills=12 score=15000 waves=3`. This is the systems-level safety net; it does NOT replace a human feel-playtest. **Headless Godot validation gotcha:** the bundled exe is nested — `~/Downloads/Godot_v4.7-stable_win64.exe/Godot_v4.7-stable_win64_console.exe` (the outer `.exe` is a FOLDER). Use the `_console.exe` for stdout; it block-buffers stdout to a pipe (interim prints only flush at exit), so redirect to a file and wait for exit rather than polling.

**Resume point:** **F5** and play the full loop (textured character + environment, full moveset, win condition, navmesh-pathing enemies, impact FX/audio). Only external blocker left: a **jump animation clip** (no Meshy jump anim yet — needs a Meshy export). The single highest-value remaining action is a **playtest for feel-tuning** (telegraph timings, halligan grip pose, difficulty curve, audio mix) — everything to date is headless-validated for crashes but not yet played.
